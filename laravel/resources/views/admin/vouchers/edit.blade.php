    ...
    <x-slot name="header">
        <div class="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-4">
            <div class="flex items-center">
                ...
                <div>
                    <h1 class="text-xl sm:text-2xl font-bold text-gray-900">Edit Voucher</h1>
                    <p class="text-sm text-gray-500 mt-1">Ubah data voucher diskon</p>
                </div>
            </div>
            ...
            <form action="{{ route('admin.vouchers.update', $voucher) }}" method="POST">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                        ...
                        <input type="text" name="code" class="form-input w-full bg-gray-100"
                    ...
                    <div>
                        ...
                        <select name="type" class="form-select w-full" required onchange="toggleVoucherFields(this)">
                            ...
                            <option value="shipping" {{ old('type', $voucher->type) == 'shipping' ? 'selected' : '' }}>
                                Potongan Ongkir</option>
                            <option value="seasonal" {{ old('type', $voucher->type) == 'seasonal' ? 'selected' : '' }}>
                                Voucher Musiman/Event</option>
                            <option value="first_purchase" {{ old('type', $voucher->type) == 'first_purchase' ? 'selected' : '' }}>Voucher Pembelian Pertama</option>
                            <option value="loyalty" {{ old('type', $voucher->type) == 'loyalty' ? 'selected' : '' }}>
                                Voucher Member/Loyal Customer</option>
                        </select>
                    </div>
                    <div>
                    ...
                    <div class="flex items-center mt-4">
                        <input type="checkbox" name="is_active" class="form-checkbox" value="1" {{ old('is_active', $voucher->is_active) ? 'checked' : '' }}>
                ...
                <div id="seasonalFields" style="display:none;" class="mt-4">
                    ...
                    <input type="text" name="event_name" class="form-input w-full"
                        value="{{ old('event_name', $voucher->event_name) }}">
                    <label class="block text-sm font-medium text-gray-700 mb-1 mt-2">Tipe Event</label>
                    <input type="text" name="event_type" class="form-input w-full"
                        value="{{ old('event_type', $voucher->event_type) }}">
                </div>
                <div id="firstPurchaseFields" style="display:none;" class="mt-4">
                    <label class="inline-flex items-center">
                        <input type="checkbox" name="first_purchase_only" class="form-checkbox" value="1" {{ old('first_purchase_only', $voucher->first_purchase_only) ? 'checked' : '' }}>
                ...
                <div id="loyaltyFields" style="display:none;" class="mt-4">
                    ...
                    <input type="text" name="member_level" class="form-input w-full"
                        value="{{ old('member_level', $voucher->member_level) }}">
                </div>
                <div class="mt-4">
                    ...
                    <textarea name="terms_and_conditions" class="form-textarea w-full"
                        rows="3">{{ old('terms_and_conditions', is_string($voucher->terms_and_conditions) ? $voucher->terms_and_conditions : (is_array($voucher->terms_and_conditions) ? implode("\n", $voucher->terms_and_conditions) : '')) }}</textarea>
                </div>
                <div class="mt-6 flex justify-end">
                    <a href="{{ route('admin.vouchers.index') }}"
                        class="px-4 py-2 bg-gray-200 text-gray-700 rounded mr-2">Batal</a>
                    <button type="submit"
                        class="px-6 py-2 bg-pink-600 text-white rounded hover:bg-pink-700 font-semibold">Simpan
                        Perubahan</button>
                </div>
            </form>
        </div>
